...
{
    private List<SetBase> mChanged = new();
    private string mCopiedItem;
    private SetBase mEditorSet;
    ...
    protected override void GameObjectUpdatedDelegate(GameObjectType type)
    {
        ...
        {
            InitEditor();
            if (mEditorSet != null && !DatabaseObject<SetBase>.Lookup.Values.Contains(mEditorSet))
            {
                mEditorSet = null;
                UpdateEditor();
            }
    ...
    private void UpdateEditor()
    {
        if (mEditorSet != null)
        {
            ...
            nudWis.Value = mEditorSet.Stats[(int)Stat.Wisdom];
            nudWisPercentage.Value = mEditorSet.PercentageStats[(int)Stat.Wisdom];

            nudVit.Value = mEditorSet.Stats[(int)Stat.Vitality];
            nudVitPercentage.Value = mEditorSet.PercentageStats[(int)Stat.Vitality];

            nudARP.Value = mEditorSet.Stats[(int)Stat.ArmorPenetration];
            nudARPPercentage.Value = mEditorSet.PercentageStats[(int)Stat.ArmorPenetration];

            nudMag.Value = mEditorSet.Stats[(int)Stat.AbilityPower];
            nudMagPercentage.Value = mEditorSet.PercentageStats[(int)Stat.AbilityPower];

            nudDef.Value = mEditorSet.Stats[(int)Stat.Defense];
            nudDefPercentage.Value = mEditorSet.PercentageStats[(int)Stat.Defense];

            nudMR.Value = mEditorSet.Stats[(int)Stat.MagicResist];
            nudMRPercentage.Value = mEditorSet.PercentageStats[(int)Stat.MagicResist];
            ...
            foreach (var item in ItemBase.Lookup.Values.OfType<ItemBase>())
            {
    ...
    private void btnAdd_Click(object sender, EventArgs e)
    {
        ...
        {
            var item = ItemBase.Get(selectedItem.Id);
    ...
    public void InitEditor()
    {
        ...
        foreach (var itm in SetBase.Lookup)
        {
            if (!string.IsNullOrEmpty(((SetBase)itm.Value).Folder) &&
                !mFolders.Contains(((SetBase)itm.Value).Folder))
            {
                mFolders.Add(((SetBase)itm.Value).Folder);
                if (!mKnownFolders.Contains(((SetBase)itm.Value).Folder))
                {
                    mKnownFolders.Add(((SetBase)itm.Value).Folder);
                }
            }
        }
        ...
        var items = SetBase.Lookup.OrderBy(p => p.Value?.Name).Select(pair => new KeyValuePair<Guid, KeyValuePair<string, string>>(pair.Key,
            new KeyValuePair<string, string>(((SetBase)pair.Value)?.Name ?? Models.DatabaseObject<SetBase>.Deleted, ((SetBase)pair.Value)?.Folder ?? ""))).ToArray();
    ...
    {
        mEditorSet.Stats[(int)Stat.Wisdom] = (int)nudWis.Value;
    }
    ...
    {
        mEditorSet.Stats[(int)Stat.ArmorPenetration] = (int)nudARP.Value;
    }
    ...
    {
        mEditorSet.Stats[(int)Stat.AbilityPower] = (int)nudMag.Value;
    }
    ...
    {
        mEditorSet.Stats[(int)Stat.MagicResist] = (int)nudMR.Value;
    }
    ...
    {
        mEditorSet.PercentageStats[(int)Stat.Wisdom] = (int)nudWisPercentage.Value;
    }
    ...
    {
        mEditorSet.PercentageStats[(int)Stat.ArmorPenetration] = (int)nudARPPercentage.Value;
    }

    private void nudMagPercentage_ValueChanged(object sender, EventArgs e)
    {
        mEditorSet.PercentageStats[(int)Stat.AbilityPower] = (int)nudMagPercentage.Value;
    }
    ...
    {
        mEditorSet.PercentageStats[(int)Stat.MagicResist] = (int)nudMRPercentage.Value;
    }
    private void cmbItems_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if (cmbItems.SelectedItem is ComboBoxItem selected)
        {
            ...
            // var item = ItemBase.Get(selected.Id);
        }
    }
