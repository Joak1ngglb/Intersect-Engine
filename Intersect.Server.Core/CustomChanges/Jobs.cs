...
namespace Intersect.Server.Entities
{
    ...
    public class PlayerJob
    {
        ...
        public void AddExperience(long amount, Player player)
        {
            ...
            while (JobExp >= GetExperienceToNextLevel(JobLevel) && JobLevel < Options.MaxJobLevel)
            {
        ...
        {
            JobLevel = Math.Clamp(newLevel, 1, Options.MaxJobLevel);
        ...
        {
            if (!Options.JobBaseExp.TryGetValue(JobType, out var baseExp))
            {
            ...
            return (long)(baseExp * Math.Pow(currentLevel, Options.GainBaseExponent));
        }
